  ...
  operations.forEach((op) => {
    ...
    switch (op.type) {
      ...
      case 'split_node':
        ...
        const childOne = currentNode.nodes[index];
        const childTwo = JSON.parse(JSON.stringify(currentNode.nodes[index]));
        ...
        if (properties) {
          if (currentNode.nodes[index + 1].object !== 'text') {
            const propertiesJSON = slateCustomToJson(properties);
      ...
      case 'merge_node':
        ...
        const one = currentNode.nodes[index - 1];
        const two = currentNode.nodes[index];
        if (one.object === 'text') {
